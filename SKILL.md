...
⚠ **两条路别混**: ① **仓内开发**(本节这些 `bun run examples/...` / `scripts/*.ts` 命令)只在 clone 的本仓里成立; ② **装包消费**(`npm i svg-infovis`)从包名引子路径, 包里既没有 `examples/` 也没有 `test/` —— 起手照抄 QUICKREF 那一段, 别去 `bun run` 本仓的示例。
