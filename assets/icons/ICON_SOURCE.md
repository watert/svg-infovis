...
本目录**不再保留任何 SVG 本地副本**。图标素材与名字索引全部来自 npm 依赖 [`lucide-static`](https://www.npmjs.com/package/lucide-static):
...
- **不走 barrel 入口**: `import * as lucide from 'lucide-static'` 是全量 re-export, bun 直跑无 tree-shake 会 eager load ~1848 个模块, 与"零 build step"约束自相矛盾
