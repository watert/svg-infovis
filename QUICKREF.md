...
| 竖线 / 斜线旁的标签横着写 | `rotate: labelAngle(pts)`(成对标签由 `pairLabels` 内置) | 竖线旁的 "Flown By" 要竖着读才顺。角度归一化到 `[-90, 90)`, **竖线一律 -90°**(文字永不倒着写)。⚠ 旋转是 `src/` 里**唯一**用 `transform` 的地方, 且审计取的是旋转后的轴对齐包围盒(`labelRect`)—— 两边读的是同一块地方, 不是双源。**例外(活体违例)**: `examples/labs/style-lab.ts:95` 的 `<g transform>`(把一格整体平移) —— 那是模板外的临时排版糖, **待拆**(平移该编进坐标, 不该借 `transform`); 所以"全仓唯一"只对 `src/` 成立, 别拿它当全仓判决 |
