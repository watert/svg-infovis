...
> 图型骨架在 `refs/recipes.md`, 函数与 API 索引在 `README.md`(仓内 / 包内), 何时用与改内核的纪律在 `SKILL.md`。
...
| `SEQ_DEFAULTS` | 列距 **132** / 盒净空 **18** / 标签净空 **12** / 标签抬升 **4** / 行距 **56** / 首行留白 **52** / 尾量 **56** / 环 **36×30** / 端点间隙 **7** / 条宽 **8** / 条高下限 **24** / 画布留白 **40** | `templates/sequence.ts` | 序列图骨架的版式缺省; **列距与右缘还会被"标签宽"顶开**(逐格取 max), 别拿 132 当"一定够"。⚠ **`barW` 与 `msgInset` 是联立守卫**: 激活条以泳道线为中心、半宽 `barW/2`, 消息端点停在距泳道线 `msgInset` 处 —— 半宽一旦够到端点就落进条里, 模板**当场抛**, 所以改任一个都要看另一个(缺省 4 < 7 才安全; 不画激活条的图 `barW` 用不上, 守卫也不查) |
...
| 网格底纹 `opts.grid` | 线: step **10** / 线宽 **1** / opacity **0.1** · 点: step **10** / 直径 **2** / opacity **0.2** · 墨色 **#999999** | `shapes/grid-pattern.ts` | ① **paper 主题自带一层**(`Theme.grid`), 不写也有; `opts.grid` 逐字段盖它(只写要改的那位), `false` 关掉 ② 深浅精度是 serialize 的 1 位小数档(写 `0.06` 变 `0.1`) —— 要更细**只能**把 alpha 编进 `color`(`#9999991a` / `rgba(153,153,153,0.06)`, rsvg 两种都认) ③ `width` 是**视觉**尺寸: 线宽内部声明 2 倍(平铺裁掉 tile 外那半) ④ 它不进 scene, 不参与 audit ⑤ `origin` = 网格块的**左上角**(缺省 `(0, 0)` = 画布原点): 铺满那块矩形与 tile **相位**都从它起算 —— 并排多块网格各铺一格、或把 `<g transform>` 拆成绝对坐标时用(活体 `examples/labs/style-lab.ts` 四格); 主题层不给这一位(块级落位与 `id` 同族, 都是作者决策) |
...
**接壤的空格保得住**(260925): 拆分后的每个 `<tspan>` 是一个独立的字符 chunk, 而渲染器在 `xml:space` 缺省档下会剥掉 chunk 的首尾空白 —— 少了这一条, `Object Type: **Airport**` 在图上成 `Object Type:Airport`(度量面量的是含空格的整串 ⇒ 盒宽与实际画的字分叉)。pin 在 `descriptor.richText` 一处: 走 spans 的 `<text>` 一律带 `xml:space="preserve"`, 无标记的单 run 老路不过那里, 字节一字不动。
...
- **`pack` 主轴间距二选一**: 心里想的是「每格 100 一档」(前缘到前缘、含整个盒) → `pitch: 100`; 只想留固定空档(缝) → `gap`。同给当场抛。⚠ 尺寸参差时 `pitch` 换算出的**每一条缝都不同**(链上每环的中心距才是那个不变量), 而"某项尺寸 > pitch"就是重叠, 也当场抛。
- **三块的缝不一样宽(38 / 58 参差) → `gap: [38, 58]`**(逐项缝, 长度必须 = 盒数 − 1)。单值 `gap` 路径一字未改, 老图字节不变。
...
| 一张图里并排铺**两种网格**(或同页 inline 多张带网格的图) | 每处显式给 `id`(`opts.grid.id` / `GridProps.id`) | 缺省 id 一律 `md-grid`, 而**重复 id 下 `url(#md-grid)` 全解析到第一个定义** —— 实测 `examples/labs/style-lab.ts grid` 四格(线10 / 线8 / 点14 / 点10)整片渲染成**线 10**, dot 格纵向量到的间距是 10 不是 14。⚠ 一图只铺一种网格时最常见的写法(不传 id)没事。**判据**: `test/style-lab-grid.test.ts`(四格 id 两两不同 + 零 `transform`) |
...
| 想按 scene 的 id / 语义槽给**产物**挂抓手(CSS 动画、外部脚本按元素找目标), 于是去数产物顺序 / 写 `:nth-child` | `tryExport(scene, { hooks: true })`(260926 起) | 缺省**关**时产物一个抓手都不吐 —— scene 的 id / tone / variant / shape 过完渲染映射全不落盘。开了: 每个元素的顶层元素带 `id`(scene id 原样透传)+ `data-kind`(`node` / `edge` / `group` / `label` / `text` / `embed`), 节点另有**恒吐**的 `data-form`(`rect` 也吐: 选择器不该猜缺省值)与**有值才吐**的 `data-tone` / `data-variant`。⚠ 它只改属性, **一个像素都不动**(六张真实图逐像素对账); 缺省关那条"逐字节不变"是对账基线, 所以开了就是**换了产物字节**; 不进 audit(门禁读 scene 不读产物) |
...
| 竖线 / 斜线旁的标签横着写 | `rotate: labelAngle(pts)`(成对标签由 `pairLabels` 内置) | 竖线旁的 "Flown By" 要竖着读才顺。角度归一化到 `[-90, 90)`, **竖线一律 -90°**(文字永不倒着写)。⚠ 旋转是**本仓自产产物里唯一**用 `transform` 的地方 —— 审计取的是旋转后的轴对齐包围盒(`labelRect`), 两边读的是同一块地方, 不是双源。(这个"唯一"260925 才立住: 并排四格底纹从前借 `<g transform="translate(…)">` 把一格整体平移, 是 `geometry/box.ts` 红线「不做嵌套坐标系」上最后一个活体违例, 现已拆成绝对坐标 + 逐格 `origin` —— tile 相位钉回各自格角, 栅格像素零差, 判据 `test/style-lab-grid.test.ts`; 外来素材 markup 里的 `transform` 是原样透传, 不计。) |
