...
// scene · 场景契约 + 可重建的几何缓存
//
// ⚠ **本文件头分两段读**: 上段是**现行宪章**(仍然有效), 下段是 **HTML / blink 量框路线的存档**
// (该路线已废弃, 论证留在仓内只为"有人再想一遍时少走一趟")。现在的决策权威是**作者手写的声明**
// (见 QUICKREF「30 秒起手」), 不是 HTML —— 别照下段去理解本文件。
//
// ── 现行宪章 ────────────────────────────────────────────────────────
// ① **scene 是缓存不是决策源**: 层 / 顺序 / 分组 / 坐标全由作者的声明给出, core 不猜意图; 这里只存
//    可重建的度量产物(bounds)、边表与诊断快照。所以本文件没有、也不许有 order / group 之类的
//    决策字段 —— 要改序就改作者的声明。
// ② **陈旧缓存不许静默出图**: 版本号 + 逐节点来源 + 源指纹三件套(机制在存档段)仍在, 判决口
//    `assertFreshForExport` 装在 `exportScene` 里 —— 门禁是真焊上的。⚠ 但**当前无触发方**
//    (`applyBounds` 全仓只有 `test/` 引用它): 主路径是作者手写 scene, 天然新鲜。机制留着是因为
//    "陈旧缓存不许静默出图"这条契约迟早要有出口。
// ③ **纯函数**: 不 mutate 入参, 一律返回新对象 —— 便于 JSON 字节比对与 golden。
//
// ── 存档: HTML 决策层 + blink 量框(已废弃; 别复活, 也别照它改本文件) ────
// 曾试图让决策活在 HTML 骨架里(DOM 父子关系与 class), scene 存 CDP 实测的 bounds。两条契约钉死了
// 它: ①决策权威单源在 HTML, 可重建的 scene 不得持有决策; ②**scene 过期是定时炸弹** —— 两类事故
// 都真发生过: (a) agent 手改 bounds 后又重测, 手改值被静默覆盖; (b) agent 改了 HTML 忘了重测,
// export 出旧 bounds。焊法 = 版本号 + 逐节点来源: HTML 落盘推进 `html_rev`, 几何写回推进
// `scene_rev`, 每节点带 `bounds_source`; 写回遇 `manual` 节点跳过并报 warning(`force` 才覆盖),
// 遇 `estimate` 节点**不许跳过**(与 manual 规则相反)。
//   · **版本号语义**: 两条 rev 共用**一条单调时间线**, 每次推进取 `max(两值) + 1`, 不是各自独立
//     自增。理由是个可复现的反例: 连续重测两次(`scene_rev` 追过 `html_rev`)之后 agent 改了 HTML ——
//     独立自增的实现会算出 `html_rev <= scene_rev` 而放行, 恰好漏掉上面第 (b) 类事故。
//   · **源指纹(260918)**: 计数只知道"落过盘", 说不出"内容变没变"(同一轮改了又改回、或没变的重存
//     都会被判陈旧)。判决换口径: `html_hash` = 当前决策源的 `sha256(语义字段)`, `scene_hash` =
//     **最近一次写回时**那一版源的指纹; 两侧都在就只比指纹(相等 = 缓存量的正是这一版源 → 放行),
//     缺任一侧才降级回计数比较并出 warning(旧文档兼容, 不破坏既有数据)。决策源由调用方交给
//     `markHtmlChanged(scene, source)`: 一段源文本(HTML / JSON)或结构化语义字段 —— core 不读盘也不
 ...
 * ⚠ `layout` / `manual` / `estimate` 三值的实际判罚依赖**已废弃的 blink 量框路线**(见文件头存档段);
 * 手写 scene 的当前主路径不产出它们。字段留着是因为陈旧检测与来源标注仍按它在写。
