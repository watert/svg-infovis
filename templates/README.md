...
**所有 `label` / content 串都认行内标记**(260925): `**粗**` / `*斜*` / `~~删~~` / `[字]{tone|#hex}` ——
模板一个字段都不用加(标记活在内容串里), 但列距 / 行距那些**按标签宽反算**的数会跟着粗体那 3% 走。
规矩与落单退字面量见 `QUICKREF.md` 的「行内标记」。
...
2. **同层相邻两盒之间的净空(56px)塞不下一条边标签** —— 标签遮罩片**高 14.8px**(260925 墨迹口径 + 内边距 3/1 之后; 旧为 23.6 → 19.6), **宽随文案**(内置示例那 4 条是 73–187px), 落在行内必压两侧盒
